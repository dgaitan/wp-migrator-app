...
class MigrateAppPullCommand {
	...
	private function write_manifest( $facts, $local, $dump, $assoc_args ) {
		...
		$themes = array();
		foreach ( array_unique( array_filter( array( $facts['template'], $facts['stylesheet'] ) ) ) as $slug ) {
			if ( is_dir( $local . '/wp-content/themes/' . $slug ) ) {
				$themes[] = 'wp-content/themes/' . $slug;
			}
		}
		if ( ! $themes && is_dir( $local . '/wp-content/themes' ) ) {
